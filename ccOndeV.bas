...
'
'  CCOndeV.BAS
' Programme gestion de protection CopyControl pour OndeV
...
Private Const BonCode = "its99Tal3.0"
Private Const BonneDLL = "ONDEV.DLL"
...
Declare Function ccdll Lib "OndeV.dll" Alias "CC32" (CC As CCMB) As Integer
...
Public Function ProtectCheck(uneFonc As Integer) As Integer
    ...
    Dim NotreCode As String, unMsgInfo As String
    Dim flag As Boolean
    ...
    If maProtectVersion = False Or maDemoVersion = True Then
        'Cas d'une version non protégée ou de démo
        ProtectCheck = 0
        If unNbAppelVerifProt = 0 Then
            If maDemoVersion = True Then
                'Cas : celui de la version démo
                unMsgInfo = "Lancement de la version DEMO du logiciel OndeV version 1.0"
                unMsgInfo = unMsgInfo + Chr(13) + Chr(13) + "===> Modification des ordonnées des feux, Enregistrement et Ouverture de fichier INDISPONIBLES"
                MsgBox unMsgInfo, vbInformation
            End If
            If maProtectVersion = False Then
                'Cas de la version protégée
